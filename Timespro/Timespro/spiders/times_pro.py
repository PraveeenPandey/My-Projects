...
class times_pro_Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(len(courses_data)):
            ...
            learn_type = ''
            if course_details['courseType'] == 'Hybrid':
                learn_type = 'Instructor Paced'
                class_timing = class_timing['CourseCovered']
            else:
                learn_type = 'Self Paced'
                class_timing = ''

            string = course_details['fees_filter'].replace('GST',"")
            fees = ''.join(e for e in string if e.isalnum())
            fees = 'Rs. '+ fees
            ...
            yield {
                ...
                'fees': fees,
                'duration': course_details['duration_filter'],
                'Learn type': learn_type,
                'class timing': class_timing,
                'Start Date': course_details['startDate'],
                'End Date': course_details['endDate']
